...
<aside class="min-w-[350px] p-2 flex flex-col items-center justify-between gap-4 bg-[#D9D9D9]">
	<div class="w-full">
		<div class="w-full flex justify-between items-center">
			<Logo />

			<div class="clipper bg-[#6B4A38] p-[3px] aspect-square rounded-[6px]">
				<Clip width="1.2rem" height="1.2rem" class="text-white fill-white" />
			</div>
		</div>
		<Separator class="my-3" />

		<div class="w-full flex items-center gap-3 rounded-full bg-[#F9F5F5] p-2">
			<img
				src={'https://api.dicebear.com/9.x/adventurer/svg?seed=' + $user?.name.split(' ')[0]}
				alt="user"
				class="w-12 aspect-square rounded-full bg-[#6B4A38]/40"
			/>
			<div>
				<p class="text-[#6B4A38] font-semibold text-lg">{$user?.name}</p>
				<p class="text-[#6B4A38] text-sm">{$user?.email}</p>
			</div>
		</div>
		...
		<div class="w-full">
			...
			<div class="w-full flex flex-col gap-2 mt-4 p-3 bg-[#C5C5C5] rounded-[8px]">
				{#if $projectListing}
					{#each $projectListing.slice(0, 5) as item}
						...
						<div on:click={() => {
							...
							<div>
								...
								<p class="text-[#6B4A38] text-sm italic">
									<span class="text-[#6B4A38]">{item.description}</span>
								</p>
							</div>
					...
					{/each}
				{/if}
				<button class="w-full p-2 bg-[#D08E6A] text-white rounded-[6px]">View All</button>
			</div>
		</div>
		...
		<div class="w-full">
			...
			{#if $Rules}
			<select bind:value={$activeRule} class="w-full p-3 my-3 bg-[#C5C5C5] rounded-[8px]">
				{#each $Rules as rule}
